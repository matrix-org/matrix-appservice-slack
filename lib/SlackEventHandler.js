...
SlackEventHandler.prototype.handleMessageEvent = function (params) {
    ...
    // which to create a ghost.)
        ...
        store.getEntryByRemoteId(msg.channel, msg.deleted_ts).then((original_event) => {
            const bot_client = this._main.getBotIntent().getClient();
            return bot_client.redactEvent(original_event.roomId, original_event.eventId);
        });
