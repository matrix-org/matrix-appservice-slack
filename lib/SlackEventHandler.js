...
SlackEventHandler.prototype.handleMessageEvent = async function (params) {
    ...
    // If it's a ephemeral event we don't have a way of relaying it.
    if (params.event.is_ephemeral) {
        return;
    }

    // If it's a message_deleted event we should redact it on the matrix side.
    if (params.event.subtype === 'message_deleted') {
        // TODO: Implement this, but it needs event id lookup.
        return;
    }
    ...
    const processMsg = msg.text || msg.files != undefined || msg.subtype === 'message_changed';
    ...
    if (msg.subtype === "file_share" && msg.file) {
        // TODO check is_public when matrix supports authenticated
        // media https://github.com/matrix-org/matrix-doc/issues/701
        // we need a user token to be able to enablePublicSharing
        if (room.getSlackUserToken()) {
            const file = this.enablePublicSharing(msg.file, room.getSlackUserToken());
            if (file) {
                msg.file = file;
