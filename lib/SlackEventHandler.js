...
SlackEventHandler.prototype.doChannelUserReplacements = async function(msg, text, token) {
...
SlackEventHandler.prototype.handleMessageEvent = async function (params) {
    const room = this._main.getRoomBySlackChannelId(params.event.channel || params.event.item.channel);
    ...
    if (params.event.type === "reaction_removed") {
    ...
    if (params.event.type === "message" && params.event.attachments) {
        for (var attachment of params.event.attachments) {
            ...
            msg.text = this.doChannelUserReplacements(msg, msg.text, token);
