...
Provisioning.commands.getlink = new Provisioning.Command({
    ...
    func: function(bridge, req, res, matrix_room_id, user_id) {
        ...
        bridge.checkLinkPermission(matrix_room_id, user_id).then((allowed) => {
            ...
            if (!room) {
                return Promise.reject({
                    code: 404,
                    text: matrix_room_id + " is not linked to Slack",
                });
            }
            ...
            (err) => {
                res.status(err.code || 500).json({error: err.text || err});
            }
        );
    }
});
