...
MatrixSlackBridge.prototype.getUserForSlackMessage = function(message) {
    var user_id = "@" + this._config.username_prefix + message.user_name +
        ":" + this._config.homeserver.server_name;

    return this._usersByUserId[user_id] = this._usersByUserId[user_id] || (() => {
        var intent = this._bridge.getIntent(user_id);

        var user = new SlackUser({intent: intent});
