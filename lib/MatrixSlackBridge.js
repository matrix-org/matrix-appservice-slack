...
MatrixSlackBridge.prototype.putLinkToStore = function(slack_channel_id, matrix_room_id) {
...
MatrixSlackBridge.prototype.upgradePendingRoom = function(room, slack_channel_id) {
    // We've just learned the channel ID for the pending-id room object
    // We'll have to
    //   * Add this as a proper entry
    //   * Add "link" entries for all the matrix room associations
    //   * Delete the old entry "PENDING" from the database

    var old_entry_id = room.toEntry().id;

    console.log("Upgrading RoomStore entry " + old_entry_id +
            " to real slack channel ID " + slack_channel_id);

    room.updateSlackChannelId(slack_channel_id);

    this.putRoomToStore(room);

    room.getMatrixRoomIds().forEach((matrix_room_id) => {
        this.putLinkToStore(slack_channel_id, matrix_room_id);
    });

    this.getRoomStore().delete({id: old_entry_id});
};
...
MatrixSlackBridge.prototype.linkRoomToMatrix = function(room, matrix_room_id) {
    ...
    if (slack_channel_id) {
        return this.putLinkToStore(slack_channel_id, matrix_room_id);
    }
    else {
        // pending-id rooms store the matrix associations directly inside
        return this.putRoomToStore(room);
    }
};
...
MatrixSlackBridge.prototype.run = function(port) {
    ...
    bridge.loadDatabases().then(() => {
        ...
        entries.forEach((entry) => {
            ...
            // pending-id rooms already have their list of matrix room IDs
            room.getMatrixRoomIds().forEach((id) => {
                this._roomsByMatrixRoomId[id] = room;
            });
        });
    }).then(() => {
        return this.getRoomStore().select({
...
Provisioning.commands.getlink = new Provisioning.Command({
    ...
    func: function(bridge, req, res, matrix_room_id) {
        ...
        res.json({
            ...
            slack_token: room.getSlackToken(),
...
Provisioning.commands.link = new Provisioning.Command({
    ...
    func: function(bridge, req, res, matrix_room_id) {
        ...
        if (params.slack_channel_id) {
            opts.slack_channel_id = params.slack_channel_id;
        }
        else if (params.slack_channel_name) {
            opts.slack_channel_name = params.slack_channel_name;
        }
        else {
            res.status(400).json({error: "Require either slack_channel_id or slack_channel_name"});
            return;
        }

        opts.slack_token = params.slack_token;
