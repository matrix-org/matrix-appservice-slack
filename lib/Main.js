...
// Returns a Promise of a SlackUser
Main.prototype.getUserForSlackMessage = function(message) {
    ...
    return store.select({id: user_id}).then((entries) => {
        var user;
        if (entries.length) {
            user = SlackUser.fromEntry(this, entries[0], intent);
        }
        else {
            user = new SlackUser({
                main: this,
            ...
            this.putUserToStore(user);
        }

        this._ghostsByUserId[user_id] = user;
        return user;
    });
};
