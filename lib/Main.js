...
Main.prototype.getUrlForMxc = function(mxc_url) {
    ...
    const url = hs.media_url !== undefined ? hs.media_url : hs.url;
    return url + "/_matrix/media/v1/download/" +
...
Main.prototype.onMatrixEvent = function(ev) {
    ...
    if (ev.type === "m.room.member"
        ...
        const intent = this._bridge.getIntent(event.state_key);
        intent.join(event.room_id).then(() => {
            return intent.sendEvent(event.room_id, "m.room.message", {
                msgtype: "m.notice",
                body: "The slack bridge doesn't support private messaging, or inviting to rooms.",
            });
        }).then(() => {
            return intent.leave(event.room_id);
        }).catch((err) => {
