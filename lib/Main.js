...
Main.prototype.getGhostForSlackMessage = async function(message) {
    ...
    const team_domain = await this.getTeamDomainForMessage(message);
    ...
    if (this._ghostsByUserId[user_id]) {
        return this._ghostsByUserId[user_id];
    }

    const intent = this._bridge.getIntent(user_id);
    const store = this.getUserStore();

    const entries = store.select({id: user_id});
    let ghost;
    if (entries.length) {
...
Main.prototype.listAllUsers = function(roomId) {
    return this._bridge.getBot().getJoinedMembers(roomId).then((joined) => {
...
Main.prototype.onMatrixEvent = function(ev) {
    ...
    if (ev.type === "m.room.redaction") {
        room.onMatrixRedaction(ev).then(
            () => endTimer({outcome: "success"}),
            (e) => {
