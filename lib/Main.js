...
Main.prototype.onMatrixEvent = function(ev) {
    ...
    if (ev.type === "m.room.member"
        ...
        && ev.sender !== myUserId) {
        log.debug(`${ev.state_key} got invite for ${ev.room_id} but we can't do DMs, warning room.`);
        ...
        intent.join(ev.room_id).then(() => {
            ...
            log.warn("Couldn't send warning to user(s) about not supporting PMs", err);
        });
        endTimer({outcome: "success"});
        return;
    }
