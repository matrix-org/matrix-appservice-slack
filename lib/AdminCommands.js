...
adminCommands.show = new AdminCommand({
    ...
    func: function(bridge, opts, args, respond) {
        ...
        respond((room.isLegacy() ? "Legacy " : "") + "Bridged Room:");
        ...
        if (room.isLegacy()) {
            ...
            if (room._upgrade_pending) respond("  UPGRADE PENDING");
        }
...
adminCommands.upgrade = new AdminCommand({
    desc: "upgrade a legacy-style room to new webhook-based routing",
    opts: {
        "!room|R": "Matrix room ID",
    },
    func: function(bridge, opts, args, respond) {
        return bridge.actionUpgrade({
            matrix_room_id: opts.room,
        }).then(
            (room) => {
                respond("Room is now " + room.getStatus());
            ...
            (e) => { respond("Cannot upgrade - " + e ) }
        );
    },
});

// Temporary command for cleaning up legacy-named users
adminCommands.oldghosts = new AdminCommand({
    desc: "list legacy-named ghosts",
    opts: {
        "!room|R": "Matrix room ID",
        "leave": "Remove legacy ghosts from the room",
    },
    func: function(bridge, opts, args, respond) {
        var room = bridge.getRoomByMatrixRoomId(opts.room);
        if (!room) return Promise.reject("No such room " + opts.room);

        var ids_to_remove = [];

        return room.listAllUsers().then((user_ids) => {
            user_ids.forEach((id) => {
                // New-style ghosts all have names in the form
                //   @_slack_TEAM_USERID
                // Old-style ghosts all have names in the form
                //   @slack_.*
                if (!id.match(/^@slack_.*/)) return;
            ...
            if (!ids_to_remove.length) {
                respond("No legacy ghosts");
                return;
            }

            if (opts.leave) return room.leaveGhosts(ids_to_remove);
        });
    },
});
