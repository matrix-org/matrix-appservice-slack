 ...
 */
function SlackHookHandler(requestLib, config, rooms, bridge, echoSuppresser) {
    ...
    if (config.users) {
        config.users.forEach(function(u) {
            var baseUrl;
            if (u.matrix.baseUrl) {
                baseUrl = u.matrix.baseUrl;
            }
            else {
                var user = u.matrix.username;
                var host = user.substring(user.indexOf(":") + 1);
                baseUrl = "https://" + host + ":8448";
            }
            self.bridge.setClientForRealUser(u.matrix.username, sdk.createClient({
                accessToken: u.matrix.access_token,
                baseUrl: baseUrl,
                userId: u.matrix.username
            }));
        });
    }

    this.suppressEcho = function(err, res) {
        if (res) {
            if (res.event_id) {
                self.echoSuppresser.suppress(res.event_id);
            }
        }
    };
...
SlackHookHandler.prototype.getIntent = function(slackUser) {
    return this.bridge.getIntent(this.usermap.matrixForSlack(slackUser));
};
...
SlackHookHandler.prototype.handle = function(params) {
    ...
    if (!this.config.slack_master_token) {
        ...
        if (params.text) {
            intent.sendText(roomID, params.text, this.suppressEcho);
        }
        return;
    }
...
SlackHookHandler.prototype.lookupAndSendMessage =
    ...
    this.requestLib.post("https://slack.com/api/channels.history", {form: params},
        ...
        if (!message.subtype) {
            intent.sendText(
                roomID,
                substitutions.slackToMatrix(message.text),
                self.suppressEcho
            );
        }
