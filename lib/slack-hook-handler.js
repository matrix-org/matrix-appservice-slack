...
SlackHookHandler.prototype.lookupAndSendMessage =
    ...
    this.requestLib.post("https://slack.com/api/channels.history", {form: params},
        ...
        // if someone mentions @YOURNICK slack sends the ID, so we need to look up the username before continuing:
        var testForName = message.text.match(/<@(\w+)>/g);
        if (null == testForName) {
            // no mentions, carry on
            sendMessage(message, intent, roomID);
        }
        else {
            // use the users.info api to convert ID to name
            var id = testForName[0].slice(2, -1)
            var options = {
                uri: 'https://slack.com/api/users.info',
                qs: {
                    token: self.config.slack_master_token,
                    user: id
                },
            ...
            rp(options).then(function (res) {
                if (res == undefined || res.user == undefined || res.user.name == undefined) {
                    console.log("users.info returned no result for " + id);
                }
                else {
                    console.log("users.info: ID " + id + " mapped to " + res.user.name)
                    message.text = message.text.replace(/<@(\w+)>/g, res.user.name);
                }
                sendMessage(message, intent, roomID);
