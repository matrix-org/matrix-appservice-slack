...
function MatrixHandler(config, rooms, requestLib, qs) {
    ...
    this.qs = qs;
    this.recentEvents = new Array(20); // store last 20 events
...
MatrixHandler.prototype.handle = function(event) {
    // basic de-dup in case matrix events are duplicated
    console.log("events: (most recent is " + this.mostRecentEvent + ")");
    for (var i = 0; i < this.recentEvents.length; i++) {
        console.log(i + " " + this.recentEvents[i]);
