...
MatrixHandler.prototype.handle = function(event) {
    ...
    var profile = this.lookupUserProfileAndSendMsg(event, options);
};

MatrixHandler.prototype.lookupUserProfileAndSendMsg = function(event, options) {
    ...
    rp(req).then(function(res) {

        if (res) {
            if (res.displayname) {
                options.body.username = res.displayname;
            ...
            if (res.avatar_url && res.avatar_url.indexOf("mxc://") === 0) {
                ...
                options.body.icon_url = "https://matrix.org/_matrix/media/v1/download/" + res.avatar_url.substring("mxc://".length);
            }
        }
        else {
            console.log("/profile API call returned no results");
        }
