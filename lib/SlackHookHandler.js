 ...
 */
function SlackHookHandler(config, bridge) {
    this.config = config;
    this.bridge = bridge;
}
...
SlackHookHandler.prototype.handle = function(params) {
    ...
    var intent = this.bridge.getIntentForSlackUsername(params.user_name);
    var roomID = room.matrix_room_id;
...
SlackHookHandler.prototype.checkAuth = function(params) {
    return params.token &&
        params.token === this.bridge.getRoomBySlackChannelId(params.channel_id).slack_token;
};
