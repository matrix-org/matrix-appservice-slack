...
SlackHookHandler.prototype.handlePost = function(room, params) {
    ...
    var token = room.getAuthToken();

    if (!token) {
        token = bridge.getTeamToken(params.team_domain);
        if (token) {
            console.log("No auth token found for " + channel_name + "; falling back on team token");
        }
    }
...
SlackHookHandler.prototype.handleAuthorize = function(room, params) {
    var oauth2 = this.bridge.getOAuth2();
    if (!oauth2) {
    ...
    return oauth2.exchangeCodeForToken({
        ...
        room.updateAuthToken(result.auth_token, result.auth_scopes);
        return this.bridge.putRoomToStore(room);
    }).then(
        () => {
            return {
                html: `
