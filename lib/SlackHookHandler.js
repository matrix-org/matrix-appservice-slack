...
SlackHookHandler.prototype.startAndListen = function(port, tls_config) {
    ...
    return new Promise((resolve, reject) => {
        createServer((request, response) => {
            ...
            request.on("end", () => {
                // All incoming slack messages will be POSTs.
                var params = qs.parse(body);
                try {
                    if (request.method === "POST") {
                        this.handle(request.url, params);
                    }
                }
                catch (e) {
 ...
 */
SlackHookHandler.prototype.handle = function(url, params) {
    console.log("Received slack webhook request to " + url + ": " + JSON.stringify(params));

    var bridge = this.bridge;

    var incReceived = function() {
        bridge.incCounter("received_messages", {side: "remote"});
    }
    ...
    var room;
    if (url.match(/^\/.{32}/)) {
        var inbound_id = url.substring(1);
        room = this.bridge.getRoomByInboundId(inbound_id);

        if (!room) {
            console.log("Ignoring message from unrecognised inbound ID: %s (%s)",
                inbound_id, channel_name
            );
            incReceived();
            bridge.incCounter("dropped_messages", {side: "remote"});
            return;
        }

        if (room._upgrade_pending) {
            delete room["_upgrade_pending"];
            this.bridge.putRoomToStore(room);
        }
    }
    else {
        room = this.bridge.getRoomBySlackChannelId(params.channel_id);
        ...
        if (!room.authenticateMessage(params)) {
            console.log("Ignoring message from slack with bad token in channel ID %s",
                params.channel_id
            );
            incReceived();
            bridge.incCounter("dropped_messages", {side: "remote"});
    ...
    this.lookupMessage(params.channel_id, params.timestamp, token).then((msg) => {
        if(undefined == msg) {
            msg = params;
        }
...
SlackHookHandler.prototype.replaceChannelIdsWithNames = function(message, token) {
    ...
    return promiseWhile(function() {
        ...
        bridge.incRemoteCallCounter("channels.info");
...
SlackHookHandler.prototype.replaceUserIdsWithNames = function(message, token) {
    ...
    return promiseWhile(() => {
        ...
        bridge.incRemoteCallCounter("users.info");
        return rp(channelsInfoApiParams).then((response) => {
...
SlackHookHandler.prototype.lookupMessage = function(channelID, timestamp, token) {
    ...
    this.bridge.incRemoteCallCounter("channels.history");
...
SlackHookHandler.prototype.fetchFileContent = function(file, token) {
    this.bridge.incRemoteCallCounter("files.sharedPublicURL");
    return rp({
        method: 'POST',
        form : {
            file: file.id,
