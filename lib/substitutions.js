...
const emoji_data = require("../external/emoji-data/emoji.json");
// Ordered
    ...
    for (var i = 0; i < emoji.length; ++i) {
        ...
        for (var j = 0; j < e.short_names.length; ++j) {
            try {
                ...
                console.error(`Failed to parse emoji ${e["short_names"][j]}`);
                console.error(err);
            }
...
// This to the best of my understanding maps the slack skin tone numbers to the
// point code used in the emoji data to get the full unified code with the
// correct skin tone modifier.
const skin_tone_mod = {2: "1F3FB",
                       3: "1F3FC",
                       4: "1F3FD",
                       5: "1F3FE",
                       6: "1F3FF"};

function slack_emoji_to_unicode(text) {
    // Original version of this function copied from https://github.com/aaronpk/Slack-IRC-Gateway
    // Copyright 2015 Aaron Parecki under the terms of the Apache 2 licence.
    const emoji_re = /\:([a-zA-Z0-9\-_\+]+)\:(?:\:([a-zA-Z0-9\-_\+]+)\:)?/g;
    const skin_re  = /skin-tone-([2-6])/;
    ...
    while(match = emoji_re.exec(text)) {
        var ed = emoji_data.find(function(el){
            return el.short_names.includes(match[1]);
        });
        if(ed) {
            const skin_tone_match = skin_re.exec(match[2]);
            let points;
            if (skin_tone_match === null) {
                points = ed.unified.split("-");
            } else {
                try {
                    points = ed.skin_variations[skin_tone_mod[skin_tone_match[1]]].unified.split("-");
                } catch(err) {
                    points = ed.unified.split("-");
                }
            }
...
var slackToMatrix = function(body, file) {
    ...
    body = slack_emoji_to_unicode(body);
    return body;
};
...
module.exports = {
    ...
    makeDiff: makeDiff,
    slack_emoji_to_unicode: slack_emoji_to_unicode
};
