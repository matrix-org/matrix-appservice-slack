...
async function getDisplayMap(main, room_id) {
    ...
    const displaymap = {};
    users = users.filter((u) => u && u[0]);
    users.forEach(user => {
        // The format is @slack_domain_userid:homeserver_domain
        const localpart = user[0].id.split(":")[0].replace("@slack", "").replace("@_slack", "");
        const slackId = localpart.split("_")[2];
        if (!slackId) {
            return;
        }
