...
SlackGhost.prototype.getDisplayName = async function(message, room) {
    const token = room.getAccessToken();
    if (message.bot_id) {
        const bot = await this.lookupBotInfo(message.bot_id, token);
        return bot && bot.name ? bot.name : message.bot_id
    }
    else {
        const user = await this.lookupUserInfo(message.user_id, token);
...
SlackGhost.prototype.updateDisplayname = async function(message, room) {
    ...
    if (!display_name) {
        const newDisplayName = await this.getDisplayName(message, room);
...
SlackGhost.prototype.lookupBotInfo = async function(bot_id, token) {
    ...
    if (!response) {
        log.error("Failed to get bot info", response);
    };

    this._user_info_cache = response;
