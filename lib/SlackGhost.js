...
SlackGhost.prototype.lookupAvatarUrl = async function(user_id, token) {
...
SlackGhost.prototype.lookupUserInfo = async function(user_id, token) {
    ...
    if (!response.user || !response.user.profile) {
        log.error(`Failed to get user profile for ${user_id}`, response);
        return;
    };
...
SlackGhost.prototype.prepareBody = function(body) {
    //TODO: This is fixing plaintext mentions, but should be refactored. See issue #110
    return body.replace(/<https:\/\/matrix\.to\/#\/@.+:.+\|(.+)>/g, "$1");
};

SlackGhost.prototype.prepareFormattedBody = function(body) {
    return Slackdown.parse(body);
};

SlackGhost.prototype.getFallbackHtml = function(room_id, replyEvent) {
    ...
    return "<mx-reply><blockquote>"
          + `<a href="https://matrix.to/#/${room_id}/${replyEvent["event_id"]}">In reply to</a>`
...
SlackGhost.prototype.getFallbackText = function(replyEvent) {
    const originalBody = (replyEvent["content"] ? replyEvent["content"]["body"] : "") || "";
    return "> <" + replyEvent["sender"] + "> " + originalBody.split("\n").join("\n> ");
};

SlackGhost.prototype.sendWithReply = async function(room_id, text, slackRoomId, slackEventTs, replyEvent, extraContent) {
    const fallbackHtml = this.getFallbackHtml(room_id, replyEvent);
    ...
    return await this.sendMessage(room_id, content, slackRoomId, slackEventTs);
};

SlackGhost.prototype.sendText = function(room_id, text, slackRoomId, slackEventTs, extraContent) {
