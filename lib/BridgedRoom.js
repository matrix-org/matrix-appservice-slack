...
BridgedRoom.prototype.onMatrixEdit = async function(message) {
    ...
    new_message = this.stripMatrixReplyFallback(new_message);
...
BridgedRoom.prototype.onMatrixMessage = async function(message) {
    ...
    if (reply !== message.event_id) {
        // We have a reply
        parentEvent = await store.getEntryByMatrixId(message.room_id, reply);
        sendMessageParams.body.thread_ts = parentEvent.remoteEventId;
    }
    ...
    if (!res || !res.ok) {
        ...
        if (parentEvent) {
            ...
            parentEvent._extras.slackThreadMessages.push(res.ts);
            await store.upsertEvent(parentEvent);
        }
...
BridgedRoom.prototype._getReplyEvent = async function(roomID, message, slackRoomID) {
    // Get parent event
    const eventStore = this._main.getEventStore();
...
BridgedRoom.prototype._handleSlackMessage = async function(message, ghost) {
    ...
    if (message.thread_ts !== undefined) {
        let replyMEvent = await this._getReplyEvent(roomID, message, slackRoomID);
        replyMEvent = this.stripMatrixReplyFallback(replyMEvent);
        return await ghost.sendWithReply(roomID, message.text, slackRoomID, eventTS, replyMEvent, undefined);
    }
    ...
    // edits
        ...
        let formatted = `<i>(edited)</i> ${before} <font color="red"> ${prev} </font> ${after} =&gt; ${before} <font color="green"> ${curr} </font> ${after}`;
        ...
        let new_body = ghost.prepareBody(new_message_rich);
        let new_formatted_body = ghost.prepareFormattedBody(new_message_rich);
        if (message.message.thread_ts !== undefined) {
            let replyEvent = await this._getReplyEvent(roomID, message.message, slackRoomID);
            replyEvent = this.stripMatrixReplyFallback(replyEvent);

            const body_fallback = ghost.getFallbackText(replyEvent);
            const formatted_fallback = ghost.getFallbackHtml(roomID, replyEvent);

            body =  body_fallback + "\n\n" + body;
            formatted =  formatted_fallback + formatted;

            new_body = body_fallback + new_body;
            new_formatted_body = formatted_fallback + new_formatted_body;
        }
        ...
        const matrixcontent = {
            ...
            "m.new_content": {
                msgtype: "m.text",
                body: new_body,
                formatted_body: new_formatted_body,
                format: "org.matrix.custom.html"
            }
        };
