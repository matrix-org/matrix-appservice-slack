...
BridgedRoom.prototype.findParentReply = async function(message) {
    ...
    let event_id = message['content']['m.relates_to'];
    if (event_id !== undefined) {
        event_id = event_id['m.in_reply_to'];
    }
    if (event_id !== undefined) {
    ...
    let next_event = await intent.getClient().fetchRoomEvent(message.room_id, event_id);

    return this.findParentReply(next_event);
...
BridgedRoom.prototype.onMatrixMessage = async function(message) {
    ...
    // TODO: Need to strip reply fallback here.
