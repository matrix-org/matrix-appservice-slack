...
BridgedRoom.prototype._handleSlackMessage = function(message, ghost) {
    ...
    if ([undefined, "bot_message", "file_comment"].includes(subtype)) {
        ...
        const edits = subsitutions.makeDiff(previous_message, new_message);

        const outtext = "(edited) " +
            edits.before + edits.prev + edits.after + " => " +
            edits.before + edits.curr + edits.after;

        const prev   = subsitutions.htmlEscape(edits.prev);
        const curr   = subsitutions.htmlEscape(edits.curr);
        const before = subsitutions.htmlEscape(edits.before);
        const after  = subsitutions.htmlEscape(edits.after);

        const formatted = "<i>(edited)</i> " + before + '<font color="red">' + prev + '</font>' + after + " =&gt; " +
