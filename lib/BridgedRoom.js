...
BridgedRoom.prototype.onSlackMessage = function(message) {
    ...
    var user = this._bridge.getUserForSlackMessage(message);

    var intent = user.getIntent();
    ...
    if (!subtype) {
        ...
        roomIDs.forEach((id) => user.sendText(id, text));
    }
        ...
        roomIDs.forEach((id) => user.sendMessage(id, message));
    }
        ...
        if (message.file._content) {
            ...
            return user.uploadContent(message.file).then((content_uri) => {
                ...
                roomIDs.forEach((id) => user.sendMessage(id, matrixMessage));
            }).finally(() => {
