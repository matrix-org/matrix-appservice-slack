...
BridgedRoom.prototype._handleSlackMessage = function(message, ghost) {
    ...
    if (message.text) {
        ...
        // Bodge: Slack's markdown is their own thing that isn't really markdown,
        // but the only parser we have for it is slackdown. However, Matrix expects
        // a variant of markdown that is in the realm of sanity so we have to convert
        // the output of slackdown back into markdown.
        // This will later be converted back into HTML.
        const slackHTML = Slackdown.parse(text);
        message.text = new Turndown().turndown(slackHTML);
    }
