...
BridgedRoom.prototype.onSlackMessage = function(message) {
    return this._main.getUserForSlackMessage(message).then((user) => {
        return user.update(message, this).then(() => {
            return this._handleSlackMessage(message, user);
        });
    });
};

BridgedRoom.prototype._handleSlackMessage = function(message, user) {
    ...
    if (!subtype || subtype === "file_comment") {
        var text = substitutions.slackToMatrix(message.text);
        return user.sendText(roomID, text).then(() => {
        ...
        return user.sendMessage(roomID, message).then(() => {
            this._main.incCounter("sent_messages", {side: "matrix"});
        });
    }
        ...
        if (message.file._content) {
            ...
            return user.uploadContent(message.file).then((content_uri) => {
                ...
                return user.sendMessage(roomID, matrixMessage);
            }).finally(() => {
                var text = substitutions.slackToMatrix(
                    message.file.initial_comment.comment
                );
                return user.sendText(roomID, text);
            }).then(() => {
                this._main.incCounter("sent_messages", {side: "matrix"});
            });
        }
