...
BridgedRoom.prototype.onMatrixEdit = async function(message) {
    ...
    if (!res || !res.ok) {
        ...
        const event = new StoredEvent(message.room_id, message.event_id, this._slack_channel_id, res.ts);
...
BridgedRoom.prototype.onMatrixMessage = async function(message) {
    ...
    const body = await substitutions.matrixToSlack(message, this._main, this._slack_team_id);
    ...
    if (reply !== message.event_id) {
        // We have a reply
        const parentStoredEvent = await store.getEntryByMatrixId(message.room_id, reply);
...
BridgedRoom.prototype.onSlackReactionAdded = async function(message) {
    if (message.user_id === this._slack_user_id) {
        return;
    }
    const ghost = await this._main.getGhostForSlackMessage(message);
    await ghost.update(message, this);
    ...
    let reaction_key = emoji.emojify(reaction, substitutions.onMissingEmoji);

    const eventStore = this._main.getEventStore();
    const event = await eventStore.getEntryByRemoteId(message.item.channel, message.item.ts);

    return ghost.sendReaction(this.getMatrixRoomId(), event.eventId, reaction_key,
                              message.item.channel, message.event_ts);
};

BridgedRoom.prototype.getMatrixReactionsBySenderByKey = async function(eventId, sender, key) {
    // The objective of this function is to hit the new
    // rooms/roomId/aggregations endpoint for all the m.reaction events with the
    // specified key.
    ...
    const roomId = this._matrix_room_id;
    const url = "/rooms/" + encodeURIComponent(roomId) + "/aggregations/" + encodeURIComponent(eventId) + "/m.annotation/m.reaction/" + encodeURIComponent(key);

    const client = this._main.getBotIntent().getClient();

    try {
        const resp = await client._http.authedRequestWithPrefix(undefined, "GET", url,
...
BridgedRoom.prototype.onSlackReactionRemoved = async function(message) {
    const eventStore = this._main.getEventStore();

    // The message.item.ts here is the ts of the *event the reaction was added to*
    const event = await eventStore.getEntryByRemoteId(message.item.channel, message.item.ts);

    const reaction_key = substitutions.slack_emoji_to_unicode(`:${message.reaction}:`);

    // We query the matrix server for all the reactions on this message with
    // that key, and then filter by sender.
    const reaction_event = await this.getMatrixReactionsBySenderByKey(event.eventId, "", reaction_key);

    if (reaction_event === undefined) {
        return;
    }

    const ghost = await this._main.getGhostForSlackMessage(message);
    const client = ghost.getIntent().getClient();

    return client.redactEvent(event.roomId, reaction_event.event_id);
};
...
BridgedRoom.prototype._handleSlackMessage = async function(message, ghost) {
    ...
    if (message.thread_ts !== undefined) {
        ...
        return ghost.sendText(roomID, message.text, slackRoomID, eventTS, extraContent);
    }
