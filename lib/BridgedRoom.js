...
function BridgedRoom(bridge, opts) {
    ...
    this._auth_token = opts.auth_token;
    this._auth_scopes = opts.auth_scopes;
...
BridgedRoom.prototype.getAuthToken = function() {
...
BridgedRoom.prototype.updateAuthToken = function(token, scopes) {
    if (this._auth_token === token &&
        this._auth_scopes.sort().join(",") === scopes.sort().join(",")) return;

    this._auth_token = token;
    this._auth_scopes = scopes;
    this._dirty = true;
};
...
BridgedRoom.fromEntry = function(bridge, entry) {
    var opts = {
        ...
        auth_token: entry.remote.auth_token,
        auth_scopes: entry.remote.auth_scopes,
    };
...
BridgedRoom.prototype.onMatrixMessage = function(message) {
    ...
    if (content && content.avatar_url && content.avatar_url.indexOf("mxc://") === 0) {
