...
function BridgedRoom(bridge, opts) {
    ...
    if (!opts.slack_channel_id && !opts.slack_channel_name) {
        throw new Error("BridgedRoom requires at least one of channel ID or name");
    }
...
BridgedRoom.prototype.toEntry = function() {
    this._dirty = false;
    var id = this._slack_channel_id || "PENDING-" + this._slack_channel_name;
    var entry = {
        id: id,
    ...
    // pending-id rooms store their list of Matrix room IDs in the 'data'
    // section because we can't make real link entities for them yet
    if (!this._slack_channel_id) {
        entry.data = {
            matrix_room_ids: this._matrix_room_ids,
        };
    }
...
BridgedRoom.prototype.snoopChannelName = function(message) {
    ...
    var channel_name = message.team_domain + ".#" + message.channel_name;
    if (channel_name != this._slack_channel_name) {
