...
import { default as substitutions, getFallbackForMissingEmoji } from "../substitutions";
...
import { BridgedRoom, ISlackChatMessagePayload, MatrixEditEvent, MatrixMessageEvent, MatrixReactionEvent,
    MatrixRedactionEvent, SlackChannelTypes, stripMatrixReplyFallback } from "../BridgedRoom";

const log = Logging.get("ChannelRoom");
interface ChannelRoomOpts {
    matrixRoomId: string;
    inboundId: string;
    slackChannelId: string;
    slackType: SlackChannelTypes;
    isPrivate: boolean;
    puppetOwner?: string;
}
...
export class ChannelRoom extends BridgedRoom {
    public get SlackChannelId(): string {
        return this.slackChannelId;
    }

    public set SlackChannelId(value: string) {
        this.dirty = this.dirty || (this.slackChannelId !== value);
        this.SlackChannelId = value;
    }

    public get SlackTeamId(): string {
        return this.team.id;
    }

    public get SlackClient(): WebClient {
    ...
    constructor(main: Main, opts: ChannelRoomOpts, private team: TeamEntry, private botClient: WebClient) {
        super(main, opts.matrixRoomId, opts.inboundId);
        this.MatrixRoomActive = true;
        this.slackChannelId = opts.slackChannelId;
        this.slackType = opts.slackType || "channel";
        this.isPrivate = opts.isPrivate;
        this.puppetOwner = opts.puppetOwner;
    }

    public updateUsingChannelInfo(channelInfo: ConversationsInfoResponse): void {
        const chan = channelInfo.channel;
        this.isPrivate = chan.is_private || false;
        if (chan.is_channel) {
            this.slackType = "channel";
        } else if (chan.is_mpim) {
            this.slackType = "mpim";
            // note: is_group is also set for mpims, so order is important
        } else if (chan.is_group) {
            this.slackType = "group";
        } else if (chan.is_im) {
            this.slackType = "im";
        } else {
            this.slackType = "unknown";
        }
        // Set this to true as we've changed the room.
        this.dirty = true;
    }
    ...
    private async getClientForRequest(userId: string): Promise<{id: string, client: WebClient}|null> {
        const puppet = await this.main.clientFactory.getClientForUserWithId(this.team.id, userId);
        if (puppet) {
            return puppet;
        }
        if (this.botClient) {
    ...
    public async onMatrixReaction(message: MatrixReactionEvent): Promise<void> {
    ...
    public async onMatrixEdit(message: MatrixEditEvent): Promise<boolean> {
        ...
        newMessage = stripMatrixReplyFallback(newMessage);
    ...
    public async onMatrixMessage(message: MatrixMessageEvent): Promise<boolean> {
        ...
        if (!this.botClient && !puppetedClient) { return false; }
    ...
    private async handleSlackMessageFile(file: ISlackFile, slackEventId: string, ghost: SlackGhost) {
        ...
        if (!authToken) {
            // To ensure that Typescript is happy
            return;
        }
        ...
        if (thumbUri && file.filetype && this.SlackClient.token) {
    ...
    private async handleSlackMessage(message: ISlackMessageEvent, ghost: SlackGhost) {
        ...
        if (message.thread_ts !== undefined && message.text) {
            ...
            if (replyMEvent) {
                replyMEvent = stripMatrixReplyFallback(replyMEvent);
        ...
        if (["bot_message", "file_comment", undefined].includes(subtype) && message.files === undefined) {
            ...
            if (message.message && message.message.thread_ts !== undefined) {
                ...
                replyEvent = stripMatrixReplyFallback(replyEvent);
                if (replyEvent) {
